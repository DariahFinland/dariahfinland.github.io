---
name: Document Understanding Tools
accessLinks:
  - label: Named entity recognition (UI)
    url: https://arkkiivi.fi/
  - label: Named entity recognition (Huggingface)
    url: https://huggingface.co/Kansallisarkisto/finbert-ner
  - label: Document type classification
    url: https://huggingface.co/jyu-digihum/findoctype
tutorialUrl: https://github.com/JYU-digihum
contacts:
  - name: Venla Poso
    url: https://www.jyu.fi/fi/henkilot/venla-poso
  - name: Ida Toivanen
    url: https://www.jyu.fi/fi/henkilot/ida-toivanen
developedBy:
  - jyvaskyla
collaborators:
  - national-archives
order: 3
---

Tools for document understanding including named entity recognition and document type classification. Most of the tool development has been conducted in collaboration with the National Archives of Finland.
